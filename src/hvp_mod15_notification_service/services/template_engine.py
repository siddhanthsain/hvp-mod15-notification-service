...
class TemplateEngine:
    ...
    def render(self, event_type: str, channel: str, variables: dict) -> dict | None:
        ...
        try:
            rendered_body = tpl["body"].format_map(_SafeDict(safe_vars))
            rendered_subject = (
                tpl["subject"].format_map(_SafeDict(safe_vars)) if tpl.get("subject") else None
            )
        except Exception as exc:
            logger.error("Template render failed event=%s channel=%s: %s", event_type, channel, exc)
