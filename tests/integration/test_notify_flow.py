...
def full_state():
    ...
    cs.upsert({"recipient_id":"ABHA-INT-001","recipient_type":"PATIENT",
               "channels":["SMS"],"phone":"+919000000001"})
    cs.upsert({"recipient_id":"adj01","recipient_type":"INSURER_ADJUDICATOR",
               "channels":["IN_APP","EMAIL"],"email":"adj01@insurer.in"})
...
def test_full_claim_approved_flow(client):
    ...
    resp = client.post("/api/v1/notifications/notify", json={
...
def test_stats_reflect_all_flows(client):
    for event in ["claim.approved", "claim.rejected"]:
        client.post("/api/v1/notifications/notify", json={
            "event_type":    event,
            "source_module": "mod04",
            "event_payload": {
